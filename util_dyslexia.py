...
def dump_sender_receiver_impatient(game: torch.nn.Module,
    ...
    with torch.no_grad():
        for batch in dataset:
            ...
            ###Now we distort the message by introducing some sort of dyslexia of 1 permutation of random distance
            nb_m = message.shape[0]
            
            message_dys = message.clone()
            
            message_lengths = find_lengths(message)
            
            mess_lengths = message_lengths.cpu().numpy()
            
            for k in range(nb_m):
                m_size = mess_lengths[k]
                perm = np.arange(m_size)
                if m_size > 2:
                    to_permutate = np.random.choice(np.arange(m_size - 1), size = 2, replace = False)
                    perm[to_permutate[0]], perm[to_permutate[1]] = perm[to_permutate[1]], perm[to_permutate[0]]
                message_dys[k,:m_size] = message[k,perm]

            output = game.receiver(message_dys, receiver_input) #### ON MODIFIE ICI LE MESSAGE EN ENTREE EN MESSAGE_DYS
            #### FIN DES CHANGEMENTS
            if not gs: output = output[0]
